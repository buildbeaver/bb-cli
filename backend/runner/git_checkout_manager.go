...
func (s *GitCheckoutManager) mirrorWith(ctx context.Context, checkout CheckoutInfo, revision plumbing.Revision, hash plumbing.Hash, log *logging.StructuredLogger) (string, error) {
	...
	// Normalize the mirrorPath for windows to match unix file separators as git doesn't work well with \ file separators
	// and this keeps the pathing manipulation(s) within this function to use in git calls.
	// i.e. Git expects:
	// - file:///C:\dev\local-mirror-host
	// - file:///C:/dev/local-mirror-host
	// but url.String() will produce file:\dev\local-mirror-host which isn't valid for git
	//
	// If we apply this change within the internal mirror logic (create mirror etc.) then we could error out on windows
	// again when we first create the path locally by passing in a unix path to it.
	//
	// This is a known bug in golang -- https://github.com/golang/go/issues/32456 and the referenced functions are not
	// exposed externally, so we manually modify this path for now.
	if runtime.GOOS == "windows" {
		mirrorPath = strings.Replace(mirrorPath, "\\", "/", -1)
	}
