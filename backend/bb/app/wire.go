...
func New(ctx context.Context, config *BBConfig) (*App, func(), error) {
	panic(wire.Build(
		...
		wire.FieldsOf(new(*BBConfig), "BBAPIConfig", "LocalBlobStoreDir", "LogFilePath", "DatabaseConfig", "RunnerLogTempDir", "SchedulerConfig", "ExecutorConfig", "LogLevels", "LogServiceConfig", "JWTConfig", "LimitsConfig", "JSON", "Verbose"),
		store.NewDatabase,
		...
		ownerships.NewNoOpOwnershipStore,
		wire.Bind(new(store.OwnershipStore), new(*ownerships.NoOpOwnershipStore)),
		legal_entities.NewBasicLegalEntityStore,
		wire.Bind(new(store.LegalEntityStore), new(*legal_entities.BasicLegalEntityStore)),
		identities.NewStore,
		...
		legal_entity.NewBasicLegalEntityService,
		wire.Bind(new(services.LegalEntityService), new(*legal_entity.BasicLegalEntityService)),
		log.NewLogService,
		wire.Bind(new(services.LogService), new(*log.LogService)),
		repo.NewBasicRepoService,
		wire.Bind(new(services.RepoService), new(*repo.BasicRepoService)),
		credential.NewJWTCredentialService,
		wire.Bind(new(services.CredentialService), new(*credential.JWTCredentialService)),
		...
		group.NewNoOpGroupService,
		wire.Bind(new(services.GroupService), new(*group.NoOpGroupService)),
		authorization.NewNoOpAuthorizationService,
		wire.Bind(new(services.AuthorizationService), new(*authorization.NoOpAuthorizationService)),
		...
		wire.Bind(new(services.AuthenticationService), new(*authentication.JWTAuthenticationService)),
